...
import { getYesterday, formatDate, formatDisplayDate, getInitialState } from './utils';
...
export default function HomePage() {
  ...
  const initialState = getInitialState(yesterdayDateStr);

  const [selectedActivities, setSelectedActivities] = useState<string[]>(initialState.activities);
  const [isSaved, setIsSaved] = useState(initialState.saved);
  ...
  const saveLog = async () => {
    ...
    try {
      ...
      const filteredLogs = logs.filter((l: { date: string }) => l.date !== yesterdayDateStr);
      filteredLogs.push({ date: yesterdayDateStr, activities: selectedActivities });

      localStorage.setItem('trackizky-logs', JSON.stringify(filteredLogs));
      setIsSaved(true);
    } catch {
